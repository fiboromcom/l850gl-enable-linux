#!/usr/bin/env bash
# l850gl-boot-recovery.sh v2.0
#
# Boot-only recovery for Fibocom L850-GL / Intel XMM7360 on Fedora.
#
# This is deliberately separate from the manual Hammer:
#   /usr/local/sbin/l850gl-manual-rescue.sh
#
# This script is for boot persistence only.
# It does not call the Hammer.
# It does not install packages.
# It does not build acpi_call from ~/src.
# It assumes acpi_call is already installed into the kernel module tree and loadable via modprobe.
#
# No AT+CFUN. No AT+COPS. No NVM writes. No sleep hook.
...
SELF_RECOVERY_WAIT="${SELF_RECOVERY_WAIT:-10}"
POST_UP_VERIFY_WAIT="${POST_UP_VERIFY_WAIT:-45}"
LOG_DIR="${LOG_DIR:-/var/log/l850gl-boot-recovery}"
STATE_DIR="${STATE_DIR:-${L850GL_HOME}/l850gl-boot-recovery-state}"
STATE_TAR="${STATE_TAR:-${L850GL_HOME}/l850gl-boot-recovery-state.tar.gz}"
...
log() { printf '[%s] [l850gl-boot-recovery v%s] %s\n' "$(date --iso-8601=seconds)" "$VERSION" "$*"; }
...
for c in systemctl rfkill lspci lsusb udevadm mmcli nmcli tar modprobe; do
  have_cmd "$c" || fail "Required command missing: $c"
done

log "Starting boot recovery"
log "CON_NAME=$CON_NAME APN=$APN XMM2USB=$XMM2USB SELF_RECOVERY_WAIT=$SELF_RECOVERY_WAIT POST_UP_VERIFY_WAIT=$POST_UP_VERIFY_WAIT"
...
log "Waiting up to ${SELF_RECOVERY_WAIT}s for normal boot/autoconnect self-recovery"
...
if usb_present; then
  ...
  log "Loading acpi_call via modprobe"
  modprobe acpi_call || fail "modprobe acpi_call failed"
  [[ -e /proc/acpi/call ]] || fail "/proc/acpi/call missing after modprobe acpi_call"
